...
def unet(pretrained_weights=None, input_size=(IMG_SIZE_Height, IMG_SIZE_Width, 1),num_class=2):
    ...
    conv7 = Conv2D(512, 5, activation='relu', padding='same', kernel_initializer='he_normal')(merge7)
    conv7 = Conv2D(512, 5, activation='relu', padding='same', kernel_initializer='he_normal')(conv7)

    up8 = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2, 2))(conv7))
    merge8 = concatenate([conv4, up8], axis=3)
    conv8 = Conv2D(256, 5, activation='relu', padding='same', kernel_initializer='he_normal')(merge8)
    conv8 = Conv2D(256, 5, activation='relu', padding='same', kernel_initializer='he_normal')(conv8)

    up9 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2, 2))(conv8))
    merge9 = concatenate([conv3, up9], axis=3)
    conv9 = Conv2D(128, 5, activation='relu', padding='same', kernel_initializer='he_normal')(merge9)
    conv9 = Conv2D(128, 5, activation='relu', padding='same', kernel_initializer='he_normal')(conv9)
    ...
    conv11 = Conv2D(32, 5, activation='relu', padding='same', kernel_initializer='he_normal')(merge11)
    conv11 = Conv2D(32, 5, activation='relu', padding='same', kernel_initializer='he_normal')(conv11)
    conv11 = Conv2D(num_class, 5, activation='relu', padding='same', kernel_initializer='he_normal')(conv11)
    ...
    model.compile(optimizer=Adam(lr=1e-2), loss=loss_function, metrics=["accuracy"])
    model.summary()
